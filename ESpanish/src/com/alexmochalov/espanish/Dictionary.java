...
public class Dictionary {
	...
	public static boolean testRus(String translation, String text, int direction) {
		...
		if (direction == 1){
			translation = translation.replaceAll("[^a-zA-Z]", "").toLowerCase();
			return translation.equals( text.replaceAll("[^a-zA-Z]", "").toLowerCase() );
		} else {
			translation = translation.replaceAll("[^а-яА-Я]", "").toLowerCase();
			return translation.equals( text.replaceAll("[^а-яА-Я]", "").toLowerCase() );
		}
		...
		while (xpp.getEventType() != XmlPullParser.END_DOCUMENT) {
			...
			case XmlPullParser.START_TAG:
				if (xpp.getName().equals("dictionary")){
					mode = "dictionary";
				} else	if (xpp.getName().equals("entry") && mode.equals("dictionary")){
					//Log.d(LOG_TAG, "getAttributeCount: " + xpp.getAttributeCount());
					for (int i = 0; i < xpp.getAttributeCount(); i++) { 
						if (xpp.getAttributeName(i).equals("text")){
							addEntry(xpp.getAttributeValue(i));
						} else 
						if (xpp.getAttributeName(i).equals("transl")){
							setTranslation(xpp.getAttributeValue(i));
						} 
					}
				} else	if (xpp.getName().equals("pronoun")){
					mode = "pronoun";
				} else	if (xpp.getName().equals("entry") && mode.equals("pronoun")){
					for (int i = 0; i < xpp.getAttributeCount(); i++) { 
						if (xpp.getAttributeName(i).equals("text")){
						...
						if (xpp.getAttributeName(i).equals("verb_ending")){
			...
			case XmlPullParser.TEXT:
				...
				if (mode.equals("Выражения") || mode.equals("Спряжения")){
					...
					ArrayList<String> itemsTextData =  new ArrayList<String>(Arrays.asList(xpp.getText().
							split("\n")));
					for (int i = 0; i < itemsTextData.size(); i++)
						itemsTextData.set(i, itemsTextData.get(i).trim());
					
					textData.get(textData.size()-1).add(new Data(itemsTextData));
				}
				*/
				break;
		default: break;
